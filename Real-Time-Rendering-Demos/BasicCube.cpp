...
int main(int argc, char* args[]) {
    ...
    if (init(&window, &windowSurface, SCREEN_WIDTH, SCREEN_HEIGHT)) {
        ...
        try {
            ...
            rtr::Pipeline* pipeline = new rtr::Pipeline {rtr::ViewPerspective(), 
                device};
            bool quit = false;
                    ...
                    if (event.type == SDL_QUIT) {
                        ...
                        if (event.cbutton.button == SDL_CONTROLLER_BUTTON_LEFTSHOULDER 
                            ...
                            switch (controllerSwitch) {
                                ...
                                case 1:
                                renderQueue.insert
                                    (std::pair<std::string, rtr::Object&>("monf", monf));
                                renderQueue.insert
                                    (std::pair<std::string, rtr::Object&>("monw", monw));
                                ...
                                case 2:
                                    ...
                                    renderQueue.insert(std::pair<std::string, rtr::Object&> ("back", back));
                                    renderQueue.insert(std::pair<std::string, rtr::Object&> ("wall", wall));
                                    renderQueue.insert(std::pair<std::string, rtr::Object&> ("walr", walr));
                                    renderQueue.insert(std::pair<std::string, rtr::Object&> ("roof", roof));
                                    renderQueue.insert(std::pair<std::string, rtr::Object&> ("floo", floo));
                            ...
                            pipeline->vertexProcessor.backface = !pipeline->vertexProcessor.backface; 
                        }
