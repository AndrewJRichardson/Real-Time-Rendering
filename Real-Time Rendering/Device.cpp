...
#include <iostream>
	...
	glm::vec2 Device::Project(glm::vec3& point, glm::mat4& transformMatrix) {

		glm::vec4 point2 = transformMatrix *  glm::vec4(point, 1);
		float x = point2.x / point2.w;
		float y = point2.y / point2.w;

		x = x * buffer->w + buffer->w / 2.0f;
		 y = -y * buffer->h + buffer->h / 2.0f;
	...
	void Device::Render(const Camera& camera, Mesh& mesh) {
		...
		glm::mat4 modelMatrix = glm::translate(glm::mat4(), mesh.position) * glm::rotate(glm::mat4(), mesh.rot, glm::vec3(1, 0, 1));
		glm::mat4 viewMatrix = glm::lookAt(camera.position, camera.position + camera.direction, camera.up);
		//glm::mat4 viewMatrix = glm::mat4_cast(camera.dir);
		...
		(this->*currentRenderMode) (transformMatrix, mesh);
	}


	void Device::RenderPoints(glm::mat4& transformMatrix, Mesh& mesh) {
		for (int i = 0; i < 8; i++) {

			glm::vec3 point1 = mesh.vertices[i];

			DrawPoint(Project(point1, transformMatrix), 0xff, 0xff, 0xff);
		}
	}



	void Device::RenderWireframes(glm::mat4& transformMatrix, Mesh& mesh) {
		for (int i = 0; i < 12; i++) {
	...
	void Device::DrawLineBresenham(glm::vec2& start, glm::vec2& end) {
